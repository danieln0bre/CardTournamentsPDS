...
public class DefaultMatchUpdateStrategy implements MatchUpdateStrategy {
    ...
    @Autowired
    public DefaultMatchUpdateStrategy(PlayerRepository playerRepository) {
    ...
    public void validatePairing(Pairing pairing) {
        ...
        if (pairing.getResult() < 0 || pairing.getResult() > 2) {
            throw new IllegalArgumentException("Invalid match result. Must be 0, 1, or 2.");
        }
    }
    ...
    public void updatePlayersResults(Pairing pairing) {
        ...
        if (pairing.getResult() == 0) {
            playerOne.setEventPoints(playerOne.getEventPoints() + 2);
        } else if (pairing.getResult() == 1) {
            playerTwo.setEventPoints(playerTwo.getEventPoints() + 2);
        } else if (pairing.getResult() == 2) {
    ...
    private void updatePlayerForBye(String playerId) {
        ...
        player.setEventPoints(player.getEventPoints() + 2);
